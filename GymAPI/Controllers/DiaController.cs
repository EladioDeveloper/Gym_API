...
namespace GymAPI.Controllers
{
    ...
    [ApiController]
    public class DiaController : ControllerBase,  IReactAdminController<Dia>
    {
        [HttpGet]
        public async Task<ActionResult<IEnumerable<Dia>>> Get(string filter = "", string range = "", string sort = "")
        {
            ...
            var count = dias.Count();

            Response.Headers.Add("Access-Control-Expose-Headers", "Content-Range");
            Response.Headers.Add("Content-Range", $"{typeof(Dia).Name.ToLower()} {from}-{to}/{count}");
            return dias;
        }
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int ID, Dia dia)
        {
            var entityId = (int)typeof(Cliente).GetProperty("ID").GetValue(dia);
            if (ID != entityId)
            {
                return BadRequest();
            }
